$(document).ready(function() {
    ...
    if (!localStorage.getItem('agree-cookie')) {
        ...
        if (cookie) cookie.classList.remove('d-none');
    }
    ...
    if (cookieButton) {
        cookieButton.addEventListener('click', function() {
            ...
            var cookie = document.getElementById('cookie');
            if (cookie) cookie.classList.add('d-none');
        });
    }
    ...
    if ($.fn.slick && $('#startSlider').length) {
        $('#startSlider').slick({
            autoplay:true,
            autoplaySpeed:3000,
            infinite:true,
    ...
    if ($.fn.slick && $('#rotationSlider').length) {
    ...
    $('.slick-prev.static').click(function() {
        ...
        if ($.fn.slick && slider.length) slider.slick('slickPrev');
    });

    $('.slick-next.static').click(function() {
        var wrapper = $(this).closest('.slider-with-arrows');
        var slider = wrapper.find('.slick-slider');
        if ($.fn.slick && slider.length) slider.slick('slickNext');
    });

    if ($.fn.slick && $('#centeredSlider').length) {
    ...
    $('.manufacture-item').on('mouseenter', function() {
        if (winWidth > 992) $(this).find('.manufacture-introtext').slideDown();
    });

    $('.manufacture-item').on('mouseleave', function() {
        if (winWidth > 992) $(this).find('.manufacture-introtext').slideUp();
    });
...
(function loadSharedSiteUi() {
    ...
    script.setAttribute('data-site-ui-loader', '1');
